...
using a top-down pathway with lateral connections:
    ...
    P5 (12×12) → upsample → add to P4 (24×24) → upsample → add to P3 (48×48)
    
The result is a single feature map at the finest scale (48×48 at 384px input)
that contains both fine-grained detail (from P3) and semantic context (from P5).

Output: (B, 256, H/8, W/8) — this feeds into both disease branches.

Channel dimensions are auto-detected from the backbone's out_channels,
so no hardcoded channel sizes are needed.
Default (EfficientNet-B4-NS): [56, 160, 448]
Legacy  (EfficientNet-B0):    [40, 112, 320]
...
class FPN(nn.Module):
    ...
    def forward(self, P3: torch.Tensor, P4: torch.Tensor, P5: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            P3: (B, P3_ch, H/8, W/8)   — fine features
                 B4-NS at 384px: (B, 56, 48, 48)
            P4: (B, P4_ch, H/16, W/16) — medium features
                 B4-NS at 384px: (B, 160, 24, 24)
            P5: (B, P5_ch, H/32, W/32) — coarse features
                 B4-NS at 384px: (B, 448, 12, 12)
        
        Returns:
            out: (B, 256, H/8, W/8) — fused multi-scale features
                  B4-NS at 384px: (B, 256, 48, 48)
        """
        # Top-down pathway: upsample coarse features and add to finer ones
        p5 = self.lat5(P5)                                             # (B, 256, H/32, W/32)
        p5_up = F.interpolate(p5, size=P4.shape[-2:], mode='nearest')  # → (B, 256, H/16, W/16)
        p4 = self.lat4(P4) + p5_up                                     # (B, 256, H/16, W/16)
        p4_up = F.interpolate(p4, size=P3.shape[-2:], mode='nearest')  # → (B, 256, H/8, W/8)
        p3 = self.lat3(P3) + p4_up                                     # (B, 256, H/8, W/8)
        
        # Smooth the final feature map
        out = self.relu(self.bn3(self.smooth3(p3)))                     # (B, 256, H/8, W/8)
